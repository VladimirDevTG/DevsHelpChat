...
http://t.me/anonymous_telegramRU"""
...
class DevsHelpChatMod(loader.Module):
    """Chat moderation
Source: http://t.me/anonymous_telegramRU"""
    strings = {
        'name': 'DevsHelpChat',
    ...
    async def newfedcmd(self, message):
        ...
        self.db.set('authorchats', 'federations', self.federations)
    ...
    async def client_ready(self, client, db):
        ...
        self.chats = db.get('authorchats', 'chats', {})
        self.warns = db.get('authorchats', 'warns', {})
        self.federations = db.get('authorchats', 'federations', {})
        ...
        self.token = db.get('authorchats', 'apitoken', False)
    ...
    async def antisexcmd(self, message):
        ...
        self.db.set('authorchats', 'chats', self.chats)
    ...
    async def pchatscmd(self, message):
        ...
        res = f"<b><u>🦊 DevsHelpChat Protection</u></b> <i>{version}</i>\n\n<i>🐼 - AntiLogspam\n🐺 - AntiHelp\n🐻 - AntiArab\n🐵 - AntiTagAll\n💋 - AntiSex\n🚪 - AntiRaid\n📯 - AntiChannel\n🪙 - AntiSpoiler\n🍓 - AntiNSFW\n⏱ - AntiFlood\n\n👋 - Welcome\n👮‍♂️ - Local warns\n💼 - Federations</i>\n\n🦊 <b><u>Chats:</u></b>\n"
        changes = False
    ...
    async def warncmd(self, message):
        ...
        if not fed:
            ...
            self.db.set('authorchats', 'warns', self.warns)
        else:
    ...
    async def gdefcmd(self, message):
        ...
        if user.id not in self.db.get('authorchats', 'gdefense', []):
            self.db.set('authorchats', 'gdefense', self.db.get('authorchats', 'gdefense', []) + [user.id])
            await utils.answer(message,
                               self.strings('defense', message).format(user.id, user.first_name if getattr(user,
                                                                                                           'first_name',
                                                                                                           None) is not None else user.title if getattr(
            ...
            self.db.set('authorchats', 'gdefense', list(set(self.db.get('authorchats', 'gdefense', [])) - set([user.id])))
            await utils.answer(message,
                               self.strings('defense', message).format(user.id, user.first_name if getattr(user,
                                                                                                           'first_name',
                                                                                                           None) is not None else user.title if getattr(
    ...
    async def gdeflistcmd(self, message):
        ...
        if not self.db.get('authorchats', 'gdefense', []):
            return await utils.answer(message, self.strings('no_defense', message))

        res = ""
        defense = self.db.get('authorchats', 'gdefense', [])
        for user in defense.copy():
            try:
                u = await self.client.get_entity(user)
            except Exception:
                self.db.set('authorchats', 'gdefense', list(set(self.db.get('authorchats', 'gdefense', [])) - set([user])))
                continue
    ...
    async def antinsfwcmd(self, message):
        ...
        if not self.token:
            async with self.client.conversation('@innoapi_auth_' + 'bot') as conv:
                ...
                await m.delete()
                await res.delete()
                self.db.set('authorchats', 'apitoken', self.token)
    ...
    async def watcher(self, message):
        ...
        try:
            ...
            if user in self.db.get('authorchats', 'gdefense', []):
                return
